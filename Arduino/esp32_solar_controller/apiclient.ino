bool api_sync()
{
  ...
  if(config.api_cellvolts)
  {
    ...
    cells_volts_real[config.cell_count-1] = doc["cell_total"];
...
bool mdnscachelookup(String hn, String &ip)
{
  ...
  IPAddress serverIp = MDNS.queryHost(String(config.api_server1));
  if(serverIp.toString() == "0.0.0.0")
  {
    String msg = "API: MDNS address resolution failed for host: " + String(config.api_server1);
    log_msg(msg);
  ...
  mdns_cache_timer = millis() + (60 * 1000);
